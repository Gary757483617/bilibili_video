...
class bilibili_video():
    def __init__(self, av_link):
        """
        :param av: a string which refers to the av number of videos in bilibili.
        """
        # self.av_num=av
        self.headers = {'user-agent': random.choice(USER_AGENT_LIST)}
        self.url = "https:" + av_link
        self.bvid = self.url.split('?')[0].split('/')[-1]
    ...
    def get_clip_nums(self):
        ...
        para_text = BeautifulSoup(requests.get(url).text, features='lxml')
        para_text = para_text.find(name="p").text

        cids = re.findall(r'\"cid\":[^,]+', para_text)[0].split(':')

        return cids

    def get_popularity(self):
        para_url = "http://api.bilibili.com/x/web-interface/view?bvid=" + self.bvid
...
def get_search_av():
    ...
    url = "https://search.bilibili.com/all?keyword=%E7%BA%A2%E8%AD%A6"
    ...
    for page in range(1, PAGE):
        ...
        # print(soup.prettify())
        videos = soup.find_all(name="li", attrs={"class": "video-item matrix"})
        for video in videos:
            ...
            av_link = video.find("a", attrs={"class": "title"})['href']
            av_nums.append(av_link)
...
def save_to_dataframe(av_nums):
    ...
    """
    video_data = pd.DataFrame(columns=['bvid', 'title', 'upload_time', 'author', 'view', 'num_danmaku',
    ...
    for i, av_num in enumerate(av_nums):
        try:
            ...
            video_data.loc[i] = [video.bvid, title, upload_time, author, view, num_danmaku, reply, favorite, coin,
                                 share, like, tags]
            print("Finished video {}/{}".format(i, total))
        except:
            continue

    video_data.to_csv("RedAlert.csv", encoding='utf-8')
...
if __name__ == '__main__':
    save_to_dataframe(get_search_av())
